...
"""
def remove_parenthesis(brackets):
    res=""
    depth=0
    for ch in brackets:
        if ch=="(": # when we see ( add it and increase depth
            if depth>0:
                res+=ch
            depth+=1
          

        else:         # when we see ) add it and decrease the depth
           depth-=1
    ...
    return res
brackets="((()))"
print(remove_parenthesis(brackets))